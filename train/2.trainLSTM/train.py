...
if use_dev:
    ...
    train_list = "./label_train_dev_list_fb.txt"
...
##  train
    ...
    acc = train_acc/sum_batch_size
    if acc > best_train_acc:
        best_train_acc = acc
        logging.info('save model')
        modelfile = '%s/model.model' % (model_dir)
        torch.save(train_module.state_dict(), modelfile)
    else:
        logging.info('!!! LR decay: %g => %g', learning_rate, learning_rate * 0.8)
        learning_rate *= 0.8
        optimizer = torch.optim.SGD(train_module.parameters(), lr=learning_rate, momentum=0.9)
##  -----------------------------------------------------------------------------------------------------------------------------
##  dev
...
logging.info('best train acc: %.3f', best_train_acc)
